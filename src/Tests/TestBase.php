...
/**
 * @file
 * Contains \Drupal\multiple_selects\Tests\TestBase.
 */
...
abstract class TestBase extends WebTestBase {
  ...
  /* @var array $modules */
  public static $modules = ['block', 'multiple_selects'];
  ...
  public function testEmptyOption($other_option = '') {
    foreach ($this->fields as $field_name => $field) {
      ...
      // an empty value.
      if ($select_type === 'multiple_selects_select' && !$multiple) {
        if ($required) {
      ...
      if ($other_option !== '') {
        ...
        // always.
        $this->setFieldValue($field_name, 'multiple_selects', $other_option);
  ...
  protected function setFieldValue($field_name, $select, $other = '') {
    ...
    if ($this->fields[$field_name]['cardinality'] == 1) {
      ...
      if ($this->fields[$field_name]['select_type'] === 'multiple_selects_select') {
